...
import static com.stormeye.event.store.audit.consumer.service.EventConstants.*;
...
public class EventAuditService {
    ...
    public EventInfo save(final String jsonEvent) {
        ...
        try {
            ...
            throw new AuditConsumerException("Error parsing JSON: " + jsonEvent, e);
        }
    ...
    public Page<EventInfo> findAllSince(final ObjectId id, final EventType eventType, final Pageable pageable) {
        ...
        final Query query = Query.query(Criteria.where(_ID).gte(id));
    ...
    public Optional<EventInfo> getApiVersion(final long eventId, final EventType eventType) {
        ...
        if (event.isPresent()) {
            ...
            Query query = Query.query(Criteria.where(_ID).lt(id)
                            .and(DATA_TYPE).is(ApiVersion.class.getSimpleName()))
    ...
    public Optional<Long> getLastEventId(final EventType eventType) {
        ...
        var query = Query.query(Criteria.where(EVENT_ID).exists(true))
                .with(Sort.by(Sort.Direction.DESC, EVENT_ID))
                .limit(1);
        ...
        if (documents.size() == 1) {
            ...
            var eventId = (Number) eventDocument.get(EVENT_ID);
    ...
    private void createIndexes() {
        ...
        for (EventType eventType : EventType.values()) {
            mongoOperations.getCollection(getCollectionName(eventType))
                    .createIndex(Indexes.ascending(TYPE, DATA_TYPE, SOURCE, EVENT_ID));
        }
    }
    ...
    private static Criteria buildCriteria(final long eventId, final Map<String, String> queryMap) {
        ...
        if (eventId == 0) {
            ...
            criteria = Criteria.where(EVENT_ID).gte(eventId);
        }
